function descriptionGenerator(item) {
  ...
  html =
    html +
    "<p style='background-color:gainsboro;color:black;width:fit-content;padding:7px;border-radius:3px;'><strong>UPC:</strong> " +
    item.upc +
    "</p>";
  ...
  html = html + "<table style='border-collapse: separate; border-spacing: 10px 5px;'>";
  if (item.desc) {
    html = html + "<tr><td><b>Description</b></td><td>" + item.desc + "</td></tr>";
  }
  if (item.caliber) {
    html = html + "<tr><td><b>Caliber</b></td><td>" + item.caliber + "</td></tr>";
  }
  if (item.action) {
    html = html + "<tr><td><b>Action</b></td><td>" + item.action + "</td></tr>";
  }
  if (item.capacity) {
    html = html + "<tr><td><b>Capacity</b></td><td>" + item.capacity + "</td></tr>";
  }
  if (item.model) {
    html = html + "<tr><td><b>Model</b></td><td>" + item.model + "</td></tr>";
  }

  item.extra.forEach((item) => {
    if (item[1]) {
      html = html + "<tr><td><b>" + item[0] + "</b></td><td>" + item[1] + "</td></tr>";
    }
  });
  ...
  html =
    html +
    "<p style='color:rgb(230, 49, 49);font-weight:bold;margin:15px 0;'>BUYER: KNOW YOUR STATE&#39;S LAWS REGARDING THE SALES, PURCHASING, AND OWNERSHIP OF FIREARMS AND FIREARM PARTS/ACCESSORIES</p>";

  html =
    html + "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>1)</strong> Please Provide the following:<br />";
  html = html + "&nbsp;&nbsp;&nbsp; A- Payment for the item and<br />";
  html =
    html + "&nbsp;&nbsp;&nbsp; B- copy of a Federal Firearms License (FFL) from a licensed dealer in your state.</p>";

  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>2)</strong> PLEASE DO NOT BID IF YOU DO NOT INTEND ON PURCHASING FROM US. YOU WILL BE REPORTED TO GUN BROKER.</p>";
  ...
  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>4)</strong> Please use Gunbroker Checkout System to record payment even if you are mailing in your payment. That way both parties have a record of payment method.</p>";

  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>5)</strong> Through Gunbroker Checkout system we do accept Visa, MasterCard, American Express, Discover, Diners Club, EnRoute, and JCB Cards.</p>";

  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>6)</strong> You can also mail-in: Personal Check, Cashier Check, Money Order. We only ship after payment is cleared which usually takes upto 14 days.</p>";

  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>7)</strong> Due to the nature of sale (firarms) our credit card merchant processing gatway require 7 days hold before the payment can clear, so we shipp the firarm within 7-10 days after payment is made using credit card.</p>";

  html =
    html + "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>8)</strong> We DO NOT accept Amazon Pay.</p>";
  ...
  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>2)</strong> Shipping is not refundable in any circumstance. Unless there was an error on our behalf, such as incorrect item was shipped based on Gunbroker.com win.</p>";

  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>3)</strong> We do not accept returns or exchanges on items, they are SOLD AS IS with our designs. All Sales are FINAL.</p>";
  ...
  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>6)</strong> Please take your time to inspect all firearms thoroughly before proceeding with the transfer. Once a new firearm is transferred to you, it is considered used, even if unfired.</p>";
  ...
  html =
    html +
    "<p style='line-height: 1.2em;margin-bottom:10px;'><strong>7)</strong> All orders are shipped within 7-10 business days using UPS upon receiving payment &amp; verified FFL information.</p>";

  html =
    html +
    "<div style='background-color:gainsboro;padding:20px;border-radius:5px;line-height: 1.2em;margin-top:30px;'>";
  html = html + "<h4 style='margin-bottom:10px;text-transform: uppercase;'>Disclaimer:</h4>";
  html =
    html +
    "<p>Buyer is solely responsible for determining that a used gun is safe to shoot. The firearms offered have not been test fired but have been given a visual inspection only for auction purposes. It is recommended that every used firearm be inspected by a qualified gunsmith before firing. By bidding, the buyer agrees to hold seller harmless against damages, injury, or death caused by defective merchandise, misuse, or disregard. By purchasing this item, you (Buyer) agree to release seller from all liability, whether criminal or civil, arising from the purchase, ownership, possession, use or misuse of this item. You further agree to all the terms set forth in this listing and by GunBroker.com.</p>";
  html = html + "</div>";
  html = html + "</div>";
  html = html + "</div>";
